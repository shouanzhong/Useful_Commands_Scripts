...
; 定义要遍历的目录
Local $directory = "D:\ProgramFiles\M01_M01_R26_GDPR\data\localfile\GDPR\APP-OS商业化应用\图库AI Studio"
...
Func TraverseDirectory($dir)
    ...
    For $i = 1 To $fileList[0]
        ...
        Else
            ; 如果是文件，检查是否是 .docx 文件
            If StringLower(StringRight($fullPath, 5)) = ".docx" Then
                ; 处理 .docx 文件
                ProcessDocxFile($fullPath)
...
; 处理 .docx 文件函数
Func ProcessDocxFile($filePath)
    ; 打开 .docx 文件
    ShellExecute($filePath)
    WinWaitActive("[CLASS:OpusApp]") ; 等待 WPS 窗口激活
    Sleep(8000) ; 等待文件完全加载
    ...
    ; 生成 .txt 文件路径
    Local $txtFilePath = StringReplace($filePath, ".docx", ".txt")
